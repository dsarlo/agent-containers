...
export class GhCodespacesProvider {
  ...
  async get(name: string): Promise<CodespaceIdentity> {
    ...
    if (!isRecord(value) || !losslessId(value.id) || !safeName(value.name) || !safeDisplay(value.environment_id) || !safeDisplay(value.state)) throw new Error('GitHub response did not contain a complete Codespace identity; refusing adoption.');
