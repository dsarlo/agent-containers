...
export function secretShaped(value: string): boolean {
  return /(?:-----BEGIN [A-Z ]*(?:PRIVATE )?KEY-----|\b(?:gh[pousr]_[A-Za-z0-9_]{20,}|github_pat_[A-Za-z0-9_]{20,}|glpat-[A-Za-z0-9_-]{20,}|sk-[A-Za-z0-9_-]{20,})\b|\b(?:authorization|auth|bearer)\s*[:=]\s*(?:bearer\s+)?\S*|\b(?:access[-_]?token|client[-_]?secret|token|password|secret|api[_-]?key|credentials?|private[-_]?key)\s*[:=]\s*\S*|--?(?:access[-_]?token|client[-_]?secret|token|password|secret|api[_-]?key|credentials?|private[-_]?key)\s*(?:=|\s+)\S+)/i.test(value);
}
...
export function redactSecretDiagnostic(value: string): string {
  return value
    ...
    .replace(/(?:--?(?:token|access-token|auth-token|oauth-token|password|secret|client-secret|api[-_]key|credentials?|private[-_]?key))(?:=|\s+)[^\s,'"\]]+/gi, '$1 [redacted]')
