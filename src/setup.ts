...
async function codespacesChecks(config: AgentContainersConfig, runner: ProcessRunner): Promise<DoctorCheck[]> {
  ...
  if (version.code === 0 && repository) {
    try {
      await new GhCodespacesProvider(runner).preflight(repository, ref);
  ...
  checks.push({ id: 'codespaces.runtime.workspace', backend: 'codespaces', phase: 'provisioned-runtime', state: 'action-required', summary: 'No exact recorded running Codespace was supplied; runtime checks were not performed.', remediation: ['Provision a Codespace explicitly after reviewing cost.', 'Run ac doctor --backend codespaces --workspace NAME.'] });
  return checks;
}
