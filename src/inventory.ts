...
import { isLocalWorkspaceMetadata, listMetadata, loadManualRecovery, type WorkspaceMetadata } from './state.js';
...
export async function inventoryWorkspaces(stateDir: string, runner: ProcessRunner, options: { probe?: boolean } = {}): Promise<WorkspaceInventory[]> {
  const entries = await listMetadata(stateDir);
  return Promise.all(entries.map((metadata) => inventoryWorkspace(stateDir, metadata, runner, options.probe === true)));
}
...
async function inventoryWorkspace(stateDir: string, metadata: WorkspaceMetadata, runner: ProcessRunner, probe: boolean): Promise<WorkspaceInventory> {
  const recovery = await recoveryState(stateDir, metadata.name);
...
async function inspectWorktree(path: string, runner: ProcessRunner, probe: boolean): Promise<ObservationState> {
  ...
  try {
    const result = await runner.run('git', ['status', '--porcelain=v1', '--untracked-files=all'], { cwd: path, kind: 'readonly-probe' });
...
async function inspectContainer(id: string | undefined, worktree: string, runner: ProcessRunner, probe: boolean): Promise<ObservationState> {
  ...
  try {
    ...
    const [observedId, label] = result.stdout.replace(/\r/g, '').trim().split('\n');
    return observedId === id && label === resolve(worktree) ? 'present' : 'ambiguous';
  } catch { return 'unknown'; }
}
...
async function lockState(stateDir: string, name: string): Promise<WorkspaceInventory['lock']> {
  try { await lstat(join(stateDir, 'locks', `${name}.lock`)); return 'present'; }
  catch (error: unknown) { return isErrno(error, 'ENOENT') ? 'none' : 'unknown'; }
}
