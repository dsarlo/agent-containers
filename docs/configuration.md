...
Configuration updates serialize cooperating Agent Containers writers through a durable publication lock and replace only the generation reviewed during onboarding. The file is checked again immediately before publication to preserve an independent writer that arrived before that boundary. POSIX does not offer an external atomic content-hash compare-and-replace, so an unrelated writer that replaces the file after that final check is not protected; do not edit the configuration outside Agent Containers while an update is in progress. First publication uses atomic no-replace publication.
