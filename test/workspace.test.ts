...
import type { ArachneConfig } from '../src/types.js';
...
test('createWorkspace uses relative Git directory pointers when Git supports them', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'arachne-workspace-'));
  ...
  const runner: ProcessRunner = {
    async run(command, args, options) {
      ...
      if (args.at(-1) === '-h') return { code: 129, stdout: '', stderr: '--relative-paths\n' };
  ...
  assert.deepEqual(calls.at(-1), { command: 'git', args: ['worktree', 'add', '--relative-paths', '-b', 'arachne/relative', join(directory, 'worktrees', 'relative'), 'main'], cwd: directory });
});
...
test('createWorkspace constructs git commands through the injected runner and writes metadata', async () => {
  ...
  assert.equal(metadata.branch, 'arachne/feature-123');
  assert.deepEqual(calls.at(-1), { command: 'git', args: ['worktree', 'add', '--relative-paths', '-b', 'arachne/feature-123', join(directory, 'worktrees', 'feature-123'), 'main'] });
  assert.equal(JSON.parse(await readFile(join(stateDir, 'workspaces', 'feature-123.json'), 'utf8')).name, 'feature-123');
});

test('createWorkspace rejects an existing Arachne workspace before invoking git worktree add', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'arachne-workspace-'));
  const stateDir = join(directory, 'state');
  await writeFile(join(directory, 'placeholder'), 'source checkout stays untouched');
  const calls: string[][] = [];
  const runner: ProcessRunner = { async run(_command, args) { calls.push(args); if (args[0] === 'show-ref') return { code: 1, stdout: '', stderr: '' }; if (args.at(-1) === '-h') return { code: 129, stdout: '', stderr: '--relative-paths\n' }; return { code: 0, stdout: `${directory}\n`, stderr: '' }; } };
  const config: ArachneConfig = { version: 1, workspace: { worktreeRoot: 'worktrees', baseBranch: 'main' }, environment: { devcontainerPath: '.devcontainer/devcontainer.json' }, commands: {} };
