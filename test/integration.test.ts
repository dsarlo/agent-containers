...
import { createWorkspace, nodeProcessRunner } from '../src/workspaces.js';
...
test('create produces an isolated worktree without altering source checkout files', { skip: !gitAvailable ? 'Git is unavailable' : !relativeWorktreeSupported ? 'installed Git does not support git worktree add --relative-paths' : false }, async () => {
  const repo = await mkdtemp(join(tmpdir(), 'arachne-git-'));
...
test('nodeProcessRunner forwards inherited terminal output without a shell', async () => {
  const result = await nodeProcessRunner.run(process.execPath, ['-e', 'process.stdout.write("arachne-terminal-smoke\\n")'], { stdio: 'inherit' });
  assert.equal(result.code, 0);
});

test('linked worktree Dev Container integration exposes the Git common directory', {
  ...
  const repo = await mkdtemp(join(tmpdir(), 'arachne-devcontainer-'));
  ...
  try {
    ...
    git('add', '.');
    git('commit', '-m', 'initial');
    git('worktree', 'add', '--relative-paths', '-b', 'arachne/integration', worktree, 'main');
    const up = spawnSync('devcontainer', ['up', '--workspace-folder', worktree, '--config', join(worktree, '.devcontainer.json'), '--log-format', 'json', '--mount-git-worktree-common-dir'], { encoding: 'utf8' });
    assert.equal(up.status, 0, up.stderr);
    const terminal = up.stdout.trim().split('\n').map((line) => { try { return JSON.parse(line) as { containerId?: string }; } catch { return {}; } }).find((line) => line.containerId);
    containerId = terminal?.containerId;
    assert.ok(containerId, 'devcontainer up must emit containerId terminal JSON');
    const exec = spawnSync('devcontainer', ['exec', '--container-id', containerId, 'git', 'rev-parse', '--git-common-dir'], { encoding: 'utf8' });
    assert.equal(exec.status, 0, exec.stderr);
    assert.match(exec.stdout, /worktrees|\.git/);
  } finally {
