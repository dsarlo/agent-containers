...
import { configurationDiff, loadConfig, saveConfigAtomic } from '../src/config.js';
...
test('doctor permits only read-only prerequisite commands and reports absent runtime explicitly', async () => {
  const calls: string[][] = [];
  const runner: ProcessRunner = { async run(command, args) { calls.push([command, ...args]); return { code: 0, stdout: args[0] === 'api' ? '{"id":1,"login":"octo"}' : 'git version 2.0', stderr: '' }; } };
