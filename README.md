...
agent-containers start <name> --yes
agent-containers stop <name> --yes
agent-containers reconcile <name>
agent-containers remove <name> --yes [--force-worktree] [--skip-container-cleanup] [--force-remote-data-loss]
```
...
- Codespaces `start`, `stop`, and `remove` are explicit gated mutations. They verify the recorded actor and immutable Codespace identity before the documented request and exact readback afterward. `reconcile` is read-only. A stop is refused while a remote command may be active, and uncertainty remains a durable recovery barrier. Codespaces removal also requires the distinct `--force-remote-data-loss` acknowledgement and leaves a tombstone rather than forgetting uncertain resources.
...
Codespaces capacity is coordinated only within one local state root. Independent hosts or state roots cannot share an exact quota; GitHub account and organization limits remain the final guard.
